...
for i, cube in enumerate(cubes):
    ...
    graph.add_node(cube_id, position=cube_position, demand=1, color=color, Ttcp=Ttcp_cube)
    Ttcp_bin = np.array(bin_transforms[color])
    Ttcp_bin[:3, 3] += [0, 0, 0.15]
    bin_position = bin_positions[color]
    graph.add_node(bin_id, position=bin_position, demand=-1, color=color, Ttcp=Ttcp_bin)
